...
public class SearchWeather extends AppCompatActivity {
    ...
    private String url = "https://restapi.amap.com/v3/weather/weatherInfo?key=9845615acd5ae84f38b67b1b34ec25f0&extensions=all&out=json";
    //使用OkHttpClient进行网络请求
    private OkHttpClient httpClient = new OkHttpClient();
    //使用Gson解析json字符串
    private Gson gson = new Gson();
    //根据不同的消息类型，处理并更新UI界面的显示内容。
...
//根据不同的消息类型，处理并更新UI界面的显示内容，同时也更新了界面上展示的天气信息。
    @SuppressLint("HandlerLeak")
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case 3:
                    ...
                    //获取SharedPreferences对象，名称为"cityweather"，可被应用程序私有访问。
                    String nowcity=sharedPreferences.getString("nowcity","");
                    //使用getString方法从SharedPreferences中获取名为"nowcity"的值，将其赋给字符串变量nowcity。
    ...
    @Override
    //在Activity创建时，从SharedPreferences中获取之前保存的城市、温度和天气信息，并将其显示在界面上。
...
//进行界面控件的初始化操作，并为按钮设置点击事件的监听器
    ...
    private void getAdcode(final String city) {
        String url = "https://restapi.amap.com/v3/geocode/geo?key=9845615acd5ae84f38b67b1b34ec25f0&address=" + city;

        final Request request = new Request.Builder().url(url).get().build();
        new Thread(new Runnable() {
            @Override
            public void run() {//创建了一个新线程
                ...
                try {
                    ...
                    if (response.isSuccessful()) {
                        ...
                        //使用JsonParser将result转换成JsonObject对象
                        JsonObject object = new JsonParser().parse(result).getAsJsonObject();
                        //从JsonObject中获取名为"geocodes"的JsonArray。
                        JsonArray array = object.get("geocodes").getAsJsonArray();
                        JsonObject info = array.get(0).getAsJsonObject();
//从JsonArray中获取第一个JsonObject，并将其赋值给变量info
                        //从info对象中获取名为"adcode"的属性值，并将其赋值给变量adcode
        ...
        new Thread(new Runnable() {
            ...
            public void run() {//创建了一个新线程
                ...
                try {
                    ...
                    //请求成功
                    if (response.isSuccessful()) {
                        String result = response.body().string();//将响应体转换为String类型的result

                        Log.i("服务器返回的结果:", result);
                        //存储解析json字符串得到的天气信息
                        List<Map<String, String>> weatherList = new ArrayList<>();//创建一个List对象weatherLis,存储解析json字符串得到的天气信息

                        //使用Gson解析result字符串，将其转换成Weather对象
                        Weather weather = gson.fromJson(result, Weather.class);
                        //从Weather对象中获取今天的天气信息。
                        Casts today = weather.getForecasts().get(0).getCasts().get(0);

                        //添加Map数据到List,创建一个Map对象map1，将今天的天气信息存储在其中
                        Map<String, String> map1 = new HashMap<>();
                        map1.put("date", "今天");//将map1添加到weatherList中。
            ...
            public void run() {
                ...
                try {
                    ...
                    if (response.isSuccessful()) {
                        ...
                        Log.i("result", result);//打印获取到的result

                        //使用JsonParser将result转换成JsonObject对象
                        JsonObject object = new JsonParser().parse(result).getAsJsonObject();
                        //从JsonObject中获取名为"geocodes"的JsonArray
                        JsonArray array = object.get("geocodes").getAsJsonArray();
                        JsonObject info = array.get(0).getAsJsonObject();//从JsonArray中获取第一个JsonObject，并将其赋值给info变量

                        //从info对象中获取名为"adcode"的属性值，并将其赋值给adcode变量。
                        String adcode = info.get("adcode").getAsString();
                        Log.i("测试获取adcode", adcode);
            ...
            public void run() {//创建了一个新线程
                ...
                try {
                    ...
                    if (response.isSuccessful()) {
                        ...
                        Log.i("服务器返回的结果:", result);
                        //存储解析json字符串得到的天气信息
                        List<Map<String, String>> weatherList = new ArrayList<>();//创建一个List对象weatherList，用于存储解析json字符串得到的天气信息。

                        //使用Gson解析result字符串，将其转换成Weather对象
                        Weather weather = gson.fromJson(result, Weather.class);
                        //获取今天天气信息
                        Casts today = weather.getForecasts().get(0).getCasts().get(0);
                        //如果SharedPreferences中保存的城市名称不为空，则更新SharedPreferences中的天气信息。
                        SharedPreferences sharedPreferences= getSharedPreferences("cityweather", Context .MODE_PRIVATE);
                        String nowcity=sharedPreferences.getString("nowcity","");//使用getString方法从SharedPreferences中获取名为"nowcity"的值，并将其赋值给字符串变量nowcity。如果没有获取到值，则将nowcity设为空字符串。
                        if(!nowcity.equals("")){
                            SharedPreferences.Editor editor = sharedPreferences.edit();//创建一个SharedPreferences.Editor对象，用于修改SharedPreferences中的数据。
                            editor.putString("weather", today.getDayweather());//使用putString方法将今天的天气情况（today.getDayweather()）存储到SharedPreferences.Editor中，键名为"weather"
                            editor.putString("tem", today.getDaytemp() + "℃");
                            editor.commit();//调用commit方法提交修改，并将新值保存到SharedPreferences中。
                        }
                        //添加Map数据到List
                        Map<String, String> map1 = new HashMap<>();
                        map1.put("date", "今天");
